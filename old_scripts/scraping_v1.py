...
class MostRevisionsSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://defensewiki.ibj.org/index.php?title=Special:MostRevisions&limit=20&offset=0"
    ]
    start_urls = [
        "https://defensewiki.ibj.org/index.php?title=Special:MostRevisions&limit=100&offset=0"
    ]
    start_urls = [
        "https://defensewiki.ibj.org/index.php?title=Special:MostRevisions&limit=2000&offset=0"
    ]  # Showing below up to 1,251 results in range #1 to #1,251.
    ...
    def parse(self, response):
        ...
        self.logger.info(
            f"\033[91mTotal valid links found: {len(valid_links)}\033[0m"
        )  # Red color

        # Step 3: Yield requests for each valid link
        for valid_link in valid_links[
            :100
        ]:  # You can remove slicing to yield all links
    ...
    def parse_page(self, response):
        ...
        page_name = response.url.split("title=")[1].split("&")[
            0
        ]  # Extract page name from URL
        filename = f"html_pages/defensewiki.ibj.org/page_{page_name}.html"

        with open(filename, "wb") as f:
            f.write(response.body)  # Write the HTML content to the file
        self.logger.info(
            f"\033[91mSaved page {response.url} to {filename}\033[0m"
        )  # Red color
