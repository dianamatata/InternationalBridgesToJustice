...
def extract_keypoint_from_file(completeness_checklist_filepath: str):
    ...
    for point in completeness_keypoints:
        ...
        if level > 0:
            ...
            full_keypoint = re.sub(r':{2,}', ':', full_keypoint) # removing all repeated colons (e.g., ::: or more) with a regex
...
def generate_description(keypoint):
    ...
    response_format = {
        "type": "json_schema",
        "json_schema": {"name": "KeypointDescription", "schema": response_format_keypoints},
    }

    answer = get_openai_response(client=client,
