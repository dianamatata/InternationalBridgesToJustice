...
from src.internationalbridgestojustice.config import Paths, MAX_CHUNK_SIZE
...
constitutions_all = load_jsonl_and_convert_to_list_of_dict(
    input_data=Paths.PATH_JSONL_FILE_WITH_CONSTITUTIONS_ALL_COUNTRIES
)
...
with open(
    Paths.PATH_JSONL_FILE_CONSTITUTION_CHUNKS, "w", encoding="utf-8"
) as jsonl_file:
    for chunk in chunks:
...
save_file(
    filename=Paths.PATH_JSONL_FILE_WITH_CONSTITUTIONS_ALL_COUNTRIES,
    content=constitutions_all,
    file_type="jsonl",
)
