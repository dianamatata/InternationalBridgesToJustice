...
from src import scrap_defensewiki_website, remove_content_field_from_tree_dict
from src.internationalbridgestojustice.config import Paths
from src import title_to_country, substring_to_country
...
save_file(
    filename=f"{Paths.PATH_FOLDER_DEFENSE_WIKI}/defensewiki_all.jsonl",
    content=dict_defensewiki,
    file_type="jsonl",
)
remove_content_field_from_tree_dict(dict_defensewiki)
save_file(
    filename=f"{Paths.PATH_FOLDER_DEFENSE_WIKI}/defensewiki_no_content.json",
    content=dict_defensewiki,
    file_type="json",
)
