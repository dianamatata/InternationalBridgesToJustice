...
def verify_claim(claim, collection, client, PROMPT_TEMPLATE):
    ...
    results = perform_similarity_search_with_country_filter(collection=collection, query_text=claim, country="Burundi", n_results=5)
    print("results")
...
chunks_selected = [chunk for chunk in chunks if chunk['metadata']['country'] == "Burundi"]
...
for chunk in range(2, len(country_extracted_claims)): # loop on chunks, here 10. analysis chunk 3
